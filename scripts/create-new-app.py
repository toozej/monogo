...
def write_app_readme(target_dir: Path, app_name: str, description: str) -> None:
    template = Template(
        textwrap.dedent(
            ...
            Scaffolded with `make new-app APP=${app_name}`. See the repo root README and
            ...
            apps/${app_name}/
              ...
              demo.sh                  # smoke-test script run via `make APP=${app_name} demo`
            ```

            ## Common workflows

            ```sh
            make test        APP=${app_name}        # run unit tests
            make local-build APP=${app_name}        # produce ./out/${app_name}
            make local-run   APP=${app_name}        # run the binary against ./apps/${app_name}/.env
            make demo        APP=${app_name}        # exercise the freshly built binary
            make release-test APP=${app_name}       # goreleaser snapshot build
            make release     APP=${app_name} TYPE=patch  # tag and push a release
            ```
            """
        ).strip()
    )
...
def write_gitignore(target_dir: Path) -> None:
    content = (
        textwrap.dedent(
            """
        # generated by make; do not commit
        c.out
        # demo artefacts
        demo-output/
        """
...
def try_run_make_app_generate(app_name: str) -> None:
    try:
        run_command(["make", "app-generate", f"APP={app_name}"], ROOT)
        print(f"Generated Docker/Air/Compose/GoReleaser configs for {app_name}")
    except subprocess.CalledProcessError:
        print(
            f"warning: 'make app-generate APP={app_name}' failed; review apps/{app_name}/ before continuing",
            file=sys.stderr,
        )
...
def scaffold(app_name: str) -> None:
    ...
    description = os.getenv("DESCRIPTION", f"{app_name} scaffolded via make new-app")
