...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compare target repository with golang-starter template."
    )
    parser.add_argument(
        "--template-dir",
        help="Path to local template directory (if omitted, clones from GitHub)",
    )
    parser.add_argument(
        "--target-dir",
        default=os.getcwd(),
    ...
    try:
        subprocess.check_call(
            [
                ...
                "--depth",
                "1",
                "https://github.com/toozej/golang-starter.git",
                temp_dir,
            ],
...
def main():
    ...
    print("Detected target project configuration:")
    ...
    if args.template_dir:
        ...
        if not os.path.isdir(template_dir):
            print(
                f"Error: Local template directory does not exist: {template_dir}",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
    ...
    try:
        ...
        for rel_file in scaffolding:
            ...
            if not os.path.exists(target_path):
                print(f"\n[ADD] {rel_file}")
                print("  File exists in template but is missing in target.")
                print(
                    "  You may want to create it with the following customized content:"
                )
                print("-" * 40)
        ...
        if not target_dockerfiles:
            ...
            for target_rel in target_dockerfiles:
                ...
                if best_match_file:
                    print(
                        f"Closest Match: {best_match_file} (Similarity: {best_match_ratio:.2%})"
                    )
                    if best_match_ratio >= 0.999:
                        print(
                            f"[MATCHED] {target_rel} is identical to {best_match_file}"
                        )
                    else:
        ...
        print(
            "Remember to NOT overwrite target Dockerfiles in their entirety, but merge updates."
        )
