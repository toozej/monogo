...
func runArchived(staleDays int) {
	...
	if reposDir != "" {
		...
		runReposModeArchived(ctx, parser, ghClient, notifier, issueCreator, reposDir, workDir, staleDays)
		return
	}

	workflowFiles, allActionRefs := resolveWorkflowFiles(parser, workDir)
	processArchived(ctx, parser, ghClient, notifier, issueCreator, workflowFiles, allActionRefs, workDir, staleDays)
}

func runReposModeArchived(ctx context.Context, parser *workflow.WorkflowParser, ghClient *github.Client, notifier *notification.NotificationManager, issueCreator *issue.IssueCreator, reposDir, workDir string, staleDays int) {
	...
	for _, repoPath := range repos {
		...
		hasIssues := processArchived(ctx, parser, ghClient, notifier, issueCreator, workflows, actionRefs, repoPath, staleDays)
		if hasIssues {
			hasAnyIssues = true
		}
	}
...
func processArchived(ctx context.Context, parser *workflow.WorkflowParser, ghClient *github.Client, notifier *notification.NotificationManager, issueCreator *issue.IssueCreator, workflowFiles []*workflow.WorkflowFile, allActionRefs []workflow.ActionRef, workDir string, staleDays int) bool {
