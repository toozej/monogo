...
- **Shared tooling:** root `Taskfile.yml`, `.pre-commit-config.yaml`, scripts, and GitHub Actions workflows. Go tool binaries are version-pinned in `tools/go-tools.tsv` and installed independently into `.tools/bin` by Task tasks.

## Build and Test Commands

- Generate configs: `task generate-all`
- List apps: `task app:list`
- Test default app: `task test`
- Test a specific app: `task test APP=golang-starter`
- Mutation-test a specific app: `task test:mutation APP=golang-starter`
- Build a local binary: `task local:build APP=golang-starter`
- Check GoReleaser config and snapshot build: `task release:test APP=golang-starter`
- Run pre-commit: `task pre-commit:run`
- Update pinned Go tools and pre-commit hooks: `task pre-commit:update`

`APP` defaults to `golang-starter`. **Use `golang-starter` to validate any change that affects all apps** — edits to `templates/app/*.tmpl`, `templates/common/*.tmpl`, root `scripts/`, `Taskfile.yml`, or the release workflow. It is the minimal in-repo starter app (no app-specific dependencies, fastest to build), so it is the quickest signal a repo-wide change is safe: exercise it first (e.g. `task release:test APP=golang-starter`, and for packaging/release changes a `goreleaser release --snapshot` build), then apply the change everywhere with `task generate-all`.
...
5. Run `task test APP=<app>` and `task release:test APP=<app>`.
...
- Releases are per-app, triggered by pushing an `apps/<app>/vX.Y.Z` tag. Use `task release APP=<app> TYPE=<major|minor|patch>` to bump and push; CI (`.github/workflows/release.yaml`) builds, signs, and publishes binaries/archives, Docker images, and the Homebrew cask.
- **Delete a bad release:** `task release:delete APP=<app> VERSION=vX.Y.Z` removes the GitHub release and deletes the tag both locally and on `origin`. `VERSION` must be passed explicitly.
...
- Use root Task tasks instead of direct tool invocations when possible.
