...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "lego-stego" {
		t.Fatalf("appName = %q, want lego-stego", appName)
	}
	if appBinary != "lego-stego" {
		t.Fatalf("appBinary = %q, want lego-stego", appBinary)
	}
	if outputDir != "docs/diagrams/lego-stego" {
		t.Fatalf("outputDir = %q, want docs/diagrams/lego-stego", outputDir)
	}
}
