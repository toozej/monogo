...
// TestGetConfigYAML exercises the end-to-end GetConfig path against the shared
// pkg/config loader: a multi-user YAML file provides the base configuration and
// an environment variable overrides a global field.
func TestGetConfigYAML(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)

	// Clean any GFL_* vars that could leak in from the environment.
	for _, k := range []string{"GFL_INTERVAL", "GFL_USER_AGENT", "GFL_VERBOSE", "GFL_ITEMS", "GFL_ZIPCODE", "GFL_DISTANCE"} {
		k := k
		_ = os.Unsetenv(k)
...
`
	...
	// Override a global field via the environment; it should win over YAML.
	t.Setenv("GFL_USER_AGENT", "env-agent")
	...
	if conf.Interval != 6*time.Hour {
		t.Errorf("expected YAML interval 6h to be preserved, got %v", conf.Interval)
	}
	if conf.UserAgent != "env-agent" {
		t.Errorf("expected env to override YAML user_agent, got %q", conf.UserAgent)
	}
	if len(conf.Users) != 1 || conf.Users[0].Name != "alice" || conf.Users[0].Distance != 15 {
		t.Errorf("expected one YAML user 'alice' with distance 15, got %+v", conf.Users)
	}
}
