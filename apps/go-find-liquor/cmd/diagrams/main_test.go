...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "go-find-liquor" {
		t.Fatalf("appName = %q, want go-find-liquor", appName)
	}
	if appBinary != "go-find-liquor" {
		t.Fatalf("appBinary = %q, want go-find-liquor", appBinary)
	}
	if outputDir != "docs/diagrams/go-find-liquor" {
		t.Fatalf("outputDir = %q, want docs/diagrams/go-find-liquor", outputDir)
	}
}
