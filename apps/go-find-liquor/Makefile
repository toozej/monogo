...
REPO_URL := https://github.com/toozej/go-find-liquor

# Docker image info
IMAGE_AUTHOR = toozej
IMAGE_NAME = go-find-liquor
IMAGE_TAG = latest

# Detect the OS and architecture
OS := $(shell uname -s)
ARCH := $(shell uname -m)
LATEST_RELEASE_URL := $(REPO_URL)/releases/latest/download/go-find-liquor_$(OS)_$(ARCH).tar.gz
...
verify: ## Verify Docker image with Cosign
	cosign verify \
	--certificate-identity-regexp '^https://github.com/toozej/go-find-liquor/\.github/workflows/(release|weekly-docker-refresh)\.yaml@refs/(tags/.*|heads/main)$$' \
	--certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
	$(IMAGE_AUTHOR)/$(IMAGE_NAME):$(IMAGE_TAG)

run: ## Run built Docker image
	-docker kill $(IMAGE_NAME)
	docker run --rm --name $(IMAGE_NAME) -v $(CURDIR)/config.yaml:/config.yaml:ro $(IMAGE_AUTHOR)/$(IMAGE_NAME):$(IMAGE_TAG)
...
local-sign: local-test ## Sign locally compiled binary with cosign keyless signing (requires interactive OIDC login)
	cosign sign-blob --bundle=$(CURDIR)/out/go-find-liquor.bundle $(CURDIR)/out/go-find-liquor --yes

local-verify: ## Verify locally compiled binary with cosign keyless verification
	cosign verify-blob \
	--certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
	--bundle $(CURDIR)/out/go-find-liquor.bundle \
	$(CURDIR)/out/go-find-liquor \
	|| (echo "WARNING: Verification failed. If this binary was signed locally (not in CI), run:" && echo "  cosign verify-blob --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' --bundle $(CURDIR)/out/go-find-liquor.bundle $(CURDIR)/out/go-find-liquor" && echo "to manually inspect the certificate identity, then verify it matches your OIDC identity." && exit 1)
...
profile-cpu: ## Generate CPU performance profile
	...
	go test -bench=. -cpuprofile=$(CURDIR)/profiles/cpu.prof $(CURDIR)/internal/search/
...
profile-mem: ## Generate memory performance profile
	...
	mkdir -p $(CURDIR)/profiles
	go test -bench=. -memprofile=$(CURDIR)/profiles/mem.prof $(CURDIR)/internal/search/
