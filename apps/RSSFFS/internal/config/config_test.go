...
// TestLoadYAML verifies that Load reads a config.yaml file (using the keys from
// config.example.yaml), applies defaults, and lets environment variables
// override YAML values.
func TestLoadYAML(t *testing.T) {
	for _, k := range []string{"RSS_READER_ENDPOINT", "RSS_READER_API_KEY", "WEB_HOST", "WEB_PORT", "RSSFFS_SINGLE_URL_MODE"} {
		k := k
		_ = os.Unsetenv(k)
	...
	yamlBody := "rss_reader_endpoint: https://yaml.example.com\n" +
		"rss_reader_api_key: yaml-key\n" +
		"web_port: 9999\n" +
	...
	// Env overrides the YAML endpoint.
	t.Setenv("RSS_READER_ENDPOINT", "https://env.example.com")

	conf, err := Load()
	if err != nil {
		t.Fatalf("Load returned error: %v", err)
	}
	if conf.RSSReaderEndpoint != "https://env.example.com" {
		t.Errorf("expected env to override YAML endpoint, got %q", conf.RSSReaderEndpoint)
	}
	if conf.RSSReaderAPIKey != "yaml-key" {
		t.Errorf("expected YAML api key, got %q", conf.RSSReaderAPIKey)
	}
	if conf.WebHost != "127.0.0.1" {
		t.Errorf("expected default web host, got %q", conf.WebHost)
	}
	if conf.WebPort != 9999 {
		t.Errorf("expected YAML web port 9999, got %d", conf.WebPort)
	}
	if !conf.SingleURLMode {
		t.Errorf("expected YAML single URL mode true, got %t", conf.SingleURLMode)
	}
}
