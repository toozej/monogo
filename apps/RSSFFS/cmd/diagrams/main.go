...
//
// This application generates architectural and component diagrams for the RSSFFS
// (RSS Feed Finder and Subscriber) project using the go-diagrams library. It creates
...
//
// This will generate:
//   - architecture.dot: High-level architecture showing RSS feed discovery and subscription flow
...
// the RSS feed discovery and subscription workflow in the RSSFFS application.
//
// The diagram illustrates:
//   - User providing a URL to scan for RSS feeds
//   - Web scraping and RSS feed discovery process
//   - RSS reader API integration for feed subscription
...
func generateArchitectureDiagram() {
	d, err := diagram.New(diagram.Filename("architecture"), diagram.Label("RSSFFS Architecture"), diagram.Direction("TB"))
	if err != nil {
		log.Fatal(err)
	}
	...
	cli := programming.Language.Go(diagram.NodeLabel("RSSFFS CLI"))
	webScraper := generic.Blank.Blank(diagram.NodeLabel("Web Scraper\n(goquery)"))
	rssDiscovery := generic.Blank.Blank(diagram.NodeLabel("RSS Feed\nDiscovery"))
	rssReader := generic.Blank.Blank(diagram.NodeLabel("RSS Reader API\n(Miniflux)"))
	config := generic.Blank.Blank(diagram.NodeLabel("Configuration\n(env/godotenv)"))
	logging := generic.Blank.Blank(diagram.NodeLabel("Logging\n(logrus)"))

	// Create connections showing the workflow
	d.Connect(user, cli, diagram.Forward())
	d.Connect(cli, config, diagram.Forward())
	d.Connect(cli, webScraper, diagram.Forward())
	d.Connect(webScraper, rssDiscovery, diagram.Forward())
	d.Connect(rssDiscovery, rssReader, diagram.Forward())
...
// relationships and dependencies between different packages in the RSSFFS project.
//
// The diagram illustrates:
//   - main.go as the entry point
//   - cmd/RSSFFS package handling CLI operations
//   - Integration with configuration, RSSFFS core logic, version, and man packages
...
func generateComponentDiagram() {
	d, err := diagram.New(diagram.Filename("components"), diagram.Label("RSSFFS Components"), diagram.Direction("LR"))
	if err != nil {
		log.Fatal(err)
	}
	...
	rootCmd := programming.Language.Go(diagram.NodeLabel("cmd/RSSFFS\nroot.go"))
	config := programming.Language.Go(diagram.NodeLabel("pkg/config\nconfig.go"))
	rssffs := programming.Language.Go(diagram.NodeLabel("internal/RSSFFS\nRSSFFS.go"))
	rssffsConfig := programming.Language.Go(diagram.NodeLabel("internal/RSSFFS\nconfig.go"))
	reader := programming.Language.Go(diagram.NodeLabel("internal/RSSFFS\nreader.go"))
	...
	d.Connect(rootCmd, rssffs, diagram.Forward())
	d.Connect(rssffs, rssffsConfig, diagram.Forward())
	d.Connect(rssffs, reader, diagram.Forward())
