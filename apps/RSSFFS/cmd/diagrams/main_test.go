...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "RSSFFS" {
		t.Fatalf("appName = %q, want RSSFFS", appName)
	}
	if appBinary != "RSSFFS" {
		t.Fatalf("appBinary = %q, want RSSFFS", appBinary)
	}
	if outputDir != "docs/diagrams/RSSFFS" {
		t.Fatalf("outputDir = %q, want docs/diagrams/RSSFFS", outputDir)
	}
}
