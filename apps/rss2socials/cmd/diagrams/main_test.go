...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "rss2socials" {
		t.Fatalf("appName = %q, want rss2socials", appName)
	}
	if appBinary != "rss2socials" {
		t.Fatalf("appBinary = %q, want rss2socials", appBinary)
	}
	if outputDir != "docs/diagrams/rss2socials" {
		t.Fatalf("outputDir = %q, want docs/diagrams/rss2socials", outputDir)
	}
}
