...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "notes-courier" {
		t.Fatalf("appName = %q, want notes-courier", appName)
	}
	if appBinary != "notes-courier" {
		t.Fatalf("appBinary = %q, want notes-courier", appBinary)
	}
	if outputDir != "docs/diagrams/notes-courier" {
		t.Fatalf("outputDir = %q, want docs/diagrams/notes-courier", outputDir)
	}
}
