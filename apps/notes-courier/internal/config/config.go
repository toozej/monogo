// Package config defines the notes-courier application configuration.
//
...
// Config represents the notes-courier application configuration. Fields are tagged
// with the environment variable names used to populate them.
type Config struct {
	// Destination selects hugo, vite, memos, or obsidian. An empty value uses SSG_TYPE.
	Destination string `env:"DESTINATION"`
	// ObsidianVaultPath is the local path of an Obsidian vault.
	ObsidianVaultPath string `env:"OBSIDIAN_VAULT_PATH"`
	// ObsidianFolder is a folder inside the vault for imported notes.
	ObsidianFolder string `env:"OBSIDIAN_FOLDER"`
	// MemosVisibility sets the visibility of new Memos notes.
	MemosVisibility string `env:"MEMOS_VISIBILITY" envDefault:"PRIVATE"`
	// Backend specifies the notes backend to use. Supported values are
	// "simplenote", "memos", "usememos", and "obsidian". Defaults to "simplenote".
