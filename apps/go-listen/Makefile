...
verify: ## Verify Docker image with Cosign
	cosign verify \
		--certificate-identity-regexp '^https://github.com/toozej/go-listen/\.github/workflows/(release|weekly-docker-refresh)\.yaml@refs/(tags/.*|heads/main)$$' \
...
local-sign: local-test ## Sign locally installed golang toolchain and cosign
	cosign sign-blob --bundle=$(CURDIR)/out/go-listen.bundle $(CURDIR)/out/go-listen --yes

local-verify: ## Verify locally compiled binary
	if test -e $(CURDIR)/out/go-listen.bundle; then \
		cosign verify-blob \
			--certificate-identity-regexp '^https://github.com/toozej/go-listen/\.github/workflows/release\.yaml@refs/tags/.*$$' \
			--certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
			--bundle $(CURDIR)/out/go-listen.bundle \
			$(CURDIR)/out/go-listen; \
	else \
		echo "No bundle found at $(CURDIR)/out/go-listen.bundle. Skipping local verification."; \
	fi
