...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "go-listen" {
		t.Fatalf("appName = %q, want go-listen", appName)
	}
	if appBinary != "go-listen" {
		t.Fatalf("appBinary = %q, want go-listen", appBinary)
	}
	if outputDir != "docs/diagrams/go-listen" {
		t.Fatalf("outputDir = %q, want docs/diagrams/go-listen", outputDir)
	}
}
