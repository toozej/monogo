...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "github-readme-gen" {
		t.Fatalf("appName = %q, want github-readme-gen", appName)
	}
	if appBinary != "readme-gen" {
		t.Fatalf("appBinary = %q, want readme-gen", appBinary)
	}
	if outputDir != "docs/diagrams/github-readme-gen" {
		t.Fatalf("outputDir = %q, want docs/diagrams/github-readme-gen", outputDir)
	}
}
