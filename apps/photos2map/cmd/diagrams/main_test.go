...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "photos2map" {
		t.Fatalf("appName = %q, want photos2map", appName)
	}
	if appBinary != "photos2map" {
		t.Fatalf("appBinary = %q, want photos2map", appBinary)
	}
	if outputDir != "docs/diagrams/photos2map" {
		t.Fatalf("outputDir = %q, want docs/diagrams/photos2map", outputDir)
	}
}
