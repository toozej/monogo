...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "gocicle" {
		t.Fatalf("appName = %q, want gocicle", appName)
	}
	if appBinary != "gocicle" {
		t.Fatalf("appBinary = %q, want gocicle", appBinary)
	}
	if outputDir != "docs/diagrams/gocicle" {
		t.Fatalf("outputDir = %q, want docs/diagrams/gocicle", outputDir)
	}
}
