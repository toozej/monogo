...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "podgrab" {
		t.Fatalf("appName = %q, want podgrab", appName)
	}
	if appBinary != "podgrab" {
		t.Fatalf("appBinary = %q, want podgrab", appBinary)
	}
	if outputDir != "docs/diagrams/podgrab" {
		t.Fatalf("outputDir = %q, want docs/diagrams/podgrab", outputDir)
	}
}
