...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "notes2ssg" {
		t.Fatalf("appName = %q, want notes2ssg", appName)
	}
	if appBinary != "notes2ssg" {
		t.Fatalf("appBinary = %q, want notes2ssg", appBinary)
	}
	if outputDir != "docs/diagrams/notes2ssg" {
		t.Fatalf("outputDir = %q, want docs/diagrams/notes2ssg", outputDir)
	}
}
