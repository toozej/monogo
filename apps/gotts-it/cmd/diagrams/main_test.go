...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "gotts-it" {
		t.Fatalf("appName = %q, want gotts-it", appName)
	}
	if appBinary != "gotts-it" {
		t.Fatalf("appBinary = %q, want gotts-it", appBinary)
	}
	if outputDir != "docs/diagrams/gotts-it" {
		t.Fatalf("outputDir = %q, want docs/diagrams/gotts-it", outputDir)
	}
}
