...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "files2prompt" {
		t.Fatalf("appName = %q, want files2prompt", appName)
	}
	if appBinary != "files2prompt" {
		t.Fatalf("appBinary = %q, want files2prompt", appBinary)
	}
	if outputDir != "docs/diagrams/files2prompt" {
		t.Fatalf("outputDir = %q, want docs/diagrams/files2prompt", outputDir)
	}
}
