...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "kmhd2playlist" {
		t.Fatalf("appName = %q, want kmhd2playlist", appName)
	}
	if appBinary != "kmhd2playlist" {
		t.Fatalf("appBinary = %q, want kmhd2playlist", appBinary)
	}
	if outputDir != "docs/diagrams/kmhd2playlist" {
		t.Fatalf("outputDir = %q, want docs/diagrams/kmhd2playlist", outputDir)
	}
}
