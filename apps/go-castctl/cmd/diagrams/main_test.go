...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "go-castctl" {
		t.Fatalf("appName = %q, want go-castctl", appName)
	}
	if appBinary != "go-castctl" {
		t.Fatalf("appBinary = %q, want go-castctl", appBinary)
	}
	if outputDir != "docs/diagrams/go-castctl" {
		t.Fatalf("outputDir = %q, want docs/diagrams/go-castctl", outputDir)
	}
}
