...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "ghreleases2rss" {
		t.Fatalf("appName = %q, want ghreleases2rss", appName)
	}
	if appBinary != "ghreleases2rss" {
		t.Fatalf("appBinary = %q, want ghreleases2rss", appBinary)
	}
	if outputDir != "docs/diagrams/ghreleases2rss" {
		t.Fatalf("outputDir = %q, want docs/diagrams/ghreleases2rss", outputDir)
	}
}
