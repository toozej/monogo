...
func TestNewManCmd_RunE(t *testing.T) {
	rootCmd := &cobra.Command{Use: "ghreleases2rss", Short: "Test Root"}
	manCmd := NewManCmd()
	...
	if !strings.Contains(output, "ghreleases2rss") {
		t.Errorf("expected man page output to contain root command name 'ghreleases2rss', got: %q", output[:min(len(output), 200)])
	}
}

func TestNewManCmd_ManPageGeneration(t *testing.T) {
	rootCmd := &cobra.Command{Use: "ghreleases2rss", Short: "My Application"}
	manCmd := NewManCmd()
	rootCmd.AddCommand(manCmd)
...
func TestNewManCmd_RunEWithRootFlags(t *testing.T) {
	rootCmd := &cobra.Command{Use: "ghreleases2rss", Short: "Flag Application"}
	...
	if !strings.Contains(output, "ghreleases2rss") {
		t.Errorf("expected man page output to contain 'ghreleases2rss', got: %q", output[:min(len(output), 200)])
	}
}
