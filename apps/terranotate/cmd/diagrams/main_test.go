...
func TestGeneratedAppMetadata(t *testing.T) {
	if appName != "terranotate" {
		t.Fatalf("appName = %q, want terranotate", appName)
	}
	if appBinary != "terranotate" {
		t.Fatalf("appBinary = %q, want terranotate", appBinary)
	}
	if outputDir != "docs/diagrams/terranotate" {
		t.Fatalf("outputDir = %q, want docs/diagrams/terranotate", outputDir)
	}
}
